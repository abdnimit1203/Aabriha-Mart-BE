// ============================================================
// DESTRUCTIVE — `npm run seed:reset`. DEV/LOCAL USE ONLY.
//
// This WIPES and reseeds Category, Product, and DeliveryRate with fixed
// sample data. It is NOT the normal seed command — `npm run seed` (seed.ts)
// is the safe one and never deletes anything. This script exists
// specifically for resetting a local dev database back to a known catalog
// state, and must never run against a database with real business data.
//
// Does NOT touch Order or User. Does NOT touch HeroBanner/Announcement/
// WelcomePopup directly — it calls the same additive ensureStorefrontDefaults()
// the safe command uses, at the end, so a fresh reset still leaves those
// populated (only if empty — never overwrites existing CMS content).
// ============================================================
...
async function seedReset() {
  ...
  console.log("DESTRUCTIVE RESET: wiping Category, Product, DeliveryRate...");
  ...
  console.log("Catalog reset complete: 4 top-level categories, 4 subcategories, 9 products, 2 delivery rates.");

  console.log("Ensuring storefront CMS defaults (additive only)...");
